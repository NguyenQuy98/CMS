...
// Defining WCP Path
if ( !defined( 'FASTOR_PLUGIN' ) )
    define( 'FASTOR_PLUGIN', trim( dirname( plugin_basename( __FILE__ ) ), '/' ) );

// Defining WCP Directory
if ( !defined( 'FASTOR_PLUGIN_DIR' ) )
    define( 'FASTOR_PLUGIN_DIR', WP_PLUGIN_DIR . '/' . FASTOR_PLUGIN );

// Defining WCP URL
if ( !defined( 'FASTOR_PLUGIN_URL' ) )
    define( 'FASTOR_PLUGIN_URL', WP_PLUGIN_URL . '/' . FASTOR_PLUGIN );
...
require_once('content_types/custom_block.php');
require_once('content_types/portfolio.php');
require_once('content_types/metaboxes/portfolio-metabox.php');

// Return Content Type Labels
function fastor_labels($singular_name, $name, $title = FALSE) {
    if( !$title )
        $title = $name;
...
function fastor_labels_tax($singular_name, $name) {
    return array(
        "name" => $name,
        "singular_name" => $singular_name,
        "search_items" => sprintf( "Search %s", $name),
...
// Return Meta Labels
function fastor_labels_meta($name, $title, $desc, $type, $std = "", $class = null, $options = null) {
    return array(
        "name" => $name,  
        "title" => $title,
        "description" => $desc,
...
// Show Meta Boxes
function fastor_show_meta_boxes($meta_boxes) {
    if (!isset($meta_boxes)) 
        return;
        
    echo '<div class="postoptions">';
    foreach ($meta_boxes as $meta_box) {
        fastor_show_meta_box($meta_box);
    }
    echo'</div>';
}

// Show Meta Box
function fastor_show_meta_box($meta_box) {
    global $post;
    ...
    if ($meta_box['type'] == "upload") { // upload image
        ...
        echo '<input value="'.stripslashes($meta_box_value).'" type="file" name="'.$meta_box['name'].'"  id="'.$meta_box['name'].'" size="50%" />';
...
// Save Post Data
function fastor_save_postdata($post_id, $meta_boxes) {
...
function fastor_show_tax_add_meta_boxes($meta_boxes) {
    if (!isset($meta_boxes))
        return;
        
    foreach ($meta_boxes as $meta_box) {
        fastor_show_tax_add_meta_box($meta_box);
    }
}

// Show Taxonomy Add Meta Box
function fastor_show_tax_add_meta_box($meta_box) {
...
// Show Taxonomy Add Meta Boxes
function fastor_show_tax_edit_meta_boxes($tag, $taxonomy, $meta_boxes) {
    if (!isset($meta_boxes))
        return;
        
    foreach ($meta_boxes as $meta_box) {
        fastor_show_tax_edit_meta_box($tag, $taxonomy, $meta_box);
    }
}

// Show Taxonomy Add Meta Box
function fastor_show_tax_edit_meta_box($tag, $taxonomy, $meta_box) {
...
// Save Tax Data
function fastor_save_taxdata( $term_id, $tt_id, $taxonomy, $meta_boxes ) {
    if (!isset($meta_boxes))
        return;
