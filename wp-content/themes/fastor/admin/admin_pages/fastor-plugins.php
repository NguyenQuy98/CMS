...
<div class="wrap about-wrap fastor-wrap">
    ...
    <h2 class="nav-tab-wrapper">
        ...
        printf( '<a href="#" class="nav-tab nav-tab-active">%s</a>', esc_html__( "Plugins", 'fastor' ) );
    ...
    <div class="fastor-section">
        <p class="about-description"><?php echo esc_html__( 'These are plugins we included inside or recommend for all design features of Fastor. You can install, activate, deactivate or update the plugins from this tab.', 'fastor' ); ?></p>
        <?php if ($install_index > 1 || $update_index > 1 || $activate_index > 1) : ?>
            <p class="about-description">
                <?php
                if ($install_index > 1)
                    printf( '<a href="%s" class="button-primary">%s</a>', admin_url( 'themes.php?page=install-required-plugins&plugin_status=install' ), esc_html__( "Click here to install plugins all together.", 'fastor' ) );
                ?>
                <?php
                if ($activate_index > 1)
                    printf( '<a href="%s" class="button-primary">%s</a>', admin_url( 'themes.php?page=install-required-plugins&plugin_status=activate' ), esc_html__( "Click here to activate plugins all together.", 'fastor' ) );
                ?>
                <?php
                if ($update_index > 1)
                    printf( '<a href="%s" class="button-primary">%s</a>', admin_url( 'themes.php?page=install-required-plugins&plugin_status=update' ), esc_html__( "Click here to update plugins all together.", 'fastor' ) );
                ?><br><br>
            </p>
        <?php endif; ?>
        <div class="fastor-install-plugins">
                        ...
                        <div class="theme-wrapper">
                            <div class="theme-screenshot">
                                ...
                                <div class="plugin-info">
                                    <?php if ( isset( $installed_plugins[ $plugin['file_path'] ] ) ) : ?>
                                        <?php printf( esc_html__( 'ver: %1s', 'fastor' ), $installed_plugins[ $plugin['file_path'] ]['Version'] ); ?>
                                    <?php elseif ( 'bundled' == $plugin['source_type'] ) : ?>
                                        <?php printf( esc_attr__( 'Available Version: %s', 'fastor' ), $plugin['version'] ); ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <h3 class="theme-name">
                            ...
                            <?php if ( isset( $plugin_action['update'] ) && $plugin_action['update'] ) : ?>
                                <div class="theme-update">
                                    <?php printf( esc_html__( 'Update Available: Version %s', 'fastor' ), $plugin['version'] ); ?>
                                </div>
                            <?php endif; ?>
                            <?php if ( isset( $plugin['required'] ) && $plugin['required'] ) : ?>
                                <div class="plugin-required">
                                    <?php esc_html_e( 'Required', 'fastor' ); ?>
                                </div>
                            <?php endif; ?>
                            <?php if ( 'active' == $plugin_status ) : ?>
                                <div class="plugin-active">
                                    <?php esc_html_e( 'Active', 'fastor' ); ?>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
