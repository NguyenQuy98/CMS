...
<?php $product = fastor_get_product(); ?>
...
<h1 class="compare-title">
    ...
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php echo esc_html__( 'Close window [X]', 'fastor' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php echo esc_html__( 'No products added in the compare table.', 'fastor' ) ?></td>
        </tr>
    ...
    <tr class="remove">
        ...
        foreach( $products as $product_id => $product ) :
            $product_class = ( $index % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product_id ?>
            <td class="<?php echo $product_class; ?>">
                <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) ) ?>" data-product_id="<?php echo $product_id; ?>"><?php echo esc_html__( 'Remove', 'fastor' ) ?> <span class="remove">x</span></a>
            </td>
            <?php
            ++$index;
        endforeach;
